...
<script>
	...
	const arr = ['待付款', '待发货', '待成团', '待收货', '交易成功', '交易关闭', '全部', '退款', '退货退款'];
	let timer = null;
	export default {
		components: {
			MyGoodsCard,
		...
		methods: {
			...
			comfirmReceipt(e) {
				...
				Dialog.confirm({
					...
					await this.$net.sendRequest("/order/receive", {
			...
			cancelOrder(item) {
				...
				Dialog.confirm({
					...
					await this.$net.sendRequest("/order/cancelOrder", {
						orderId: item.orderNoString
					});
					this.refresh();
					// on confirm
				}).catch(() => {
			...
			updateTime() {
				timer = setInterval(() => {
					...
					this.pageData[this.loadIndex].list.forEach((item, index) => {
						...
						this.$set(this.pageData[this.loadIndex].list[index], 'time', addZero(Math.floor(seconds / 3600)) + ":" +
			...
			async comfirmPayOrder(e) {
				...
				//限制购买
				const islimit = await Api.isLimitBuy(item.items.map(item => {
					return {
						id: item.productId,
			...
			deleteOrder(e) {
				...
				Dialog.confirm({
					...
					await this.$net.sendRequest("/order/delete", {
						orderIds: item.orderNoString
					});
					this.refresh();
					// on confirm
				}).catch(() => {
